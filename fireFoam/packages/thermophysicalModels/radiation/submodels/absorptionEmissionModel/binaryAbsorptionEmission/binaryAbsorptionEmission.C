...
{
//<fmglobal>
    if ( model1_->isGrey() != model2_->isGrey() )
    {
	word word1 = model1_->isGrey()? "gray" : "nongray";
	word word2 = model2_->isGrey()? "gray" : "nongray";
	FatalErrorInFunction
	    << " Incompatible models specified in binaryAbsorptionEmission: \n"
	    << "   model1 is " << word1 <<"\n"
	    << "   model2 is " << word2 <<"\n"
	    << exit(FatalError);
    }
