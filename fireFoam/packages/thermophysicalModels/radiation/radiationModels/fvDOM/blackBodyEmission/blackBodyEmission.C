...
//<luwi--need to also update at boundaries>
        volScalarField::Boundary& deltaLambdaTBf = deltaLambdaT.ref().boundaryFieldRef();

        forAll(deltaLambdaTBf, patchi)
        {
            fvPatchScalarField& deltaLambdaTPf = deltaLambdaTBf[patchi];

            if (!deltaLambdaTPf.coupled())
            {
                const scalarField& Tpf = T.boundaryField()[patchi];

                forAll(deltaLambdaTPf, facei)
                {
                    const scalar T1 = fLambdaT(band[1]*Tpf[facei]);
                    const scalar T2 = fLambdaT(band[0]*Tpf[facei]);

                    deltaLambdaTPf[facei] *= T1 - T2;
                }
            }
        }
//</luwi>
...
) const
{
    ...
    {
        scalar T1 = fLambdaT(band[1]*T);
        scalar T2 = fLambdaT(band[0]*T);
        fr = T1 - T2;
...
{
    enFracLambda = deltaLambdaT(T_, band);
}
