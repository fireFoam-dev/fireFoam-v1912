...
:
    ...
    ILambda_(nLambda),
    myRayId_(rayId),
    qrLambda_(nLambda), // luwi
    qinLambda_(nLambda), // luwi
    qemLambda_(nLambda), // luwi
    convLambda_(nLambda) // luwi
{
...
Foam::scalar Foam::radiation::radiativeIntensityRay::correct()
{
    ...
    label nbandsConverged = 0; // luwi

    forAll(ILambda_, lambdaI)
    {
//<fmglobal>	    
	//skip band if already converged
	if (convLambda_[lambdaI])
	{
	    nbandsConverged++;
	    continue;
	}
//</fmglobal>
        ...
        const volScalarField sigmaEff(scatter_.sigmaEff(lambdaI,myRayId_,omega_)); // ankur
        // reset boundary spectral heat fluxes to zero
        qrLambda_[lambdaI].boundaryFieldRef() = 0.0; // luwi
        qinLambda_[lambdaI].boundaryFieldRef() = 0.0; // luwi
        qemLambda_[lambdaI].boundaryFieldRef() = 0.0; // luwi
        ...
        ==
           ...
           *(
                // Remove aDisp from k for gas emissivity // luwi
                (k - absorptionEmission_.aDisp(lambdaI))
                *physicoChemical::sigma*pow4(dom_.T())*dom_.T4fac()*dom_.enFracLambda(lambdaI) // luwi
               // *blackBody_.bLambda(lambdaI)   // ankur, the above expression computes banded emmision

              + absorptionEmission_.E(lambdaI)/4
            )

            + omega_* dom_.inScatEnergy(lambdaI,myRayId_) // luwi
        );
...
//<fmglobal>
	const label numSolIters =
	    ILambdaSol.nIterations();
	
	convLambda_[lambdaI] =
	    (
		numSolIters==0
	     || initialRes < dom_.tolerance()
	    );
	nbandsConverged += convLambda_[lambdaI];
//</fmglobal>
    }
//<fmglobal>
    if (nbandsConverged==nLambda_)
    {
	maxResidual = 0.; 
    }
//</fmglobal>
...
//<fmglobal>
void Foam::radiation::radiativeIntensityRay::resetConvLambda()
{
    forAll(convLambda_, lambdaI)
    {
	convLambda_[lambdaI] = 0;
    }
}
//</fmglobal>
