...
#if 1
	//<fmglobal>
	// Water droplets only absorb radiation...
	const label cellP = this->cell();
	const scalar aP = this->areaP();
	const scalar nP = this->nParticle_;
        // sigmaAbsG = product of single-droplet absorption cross-section and cell irradiant flux
	// NOTE: radCalc must be called below even when coupledRadiation is false -- updates droplet
	//       radiation properties
	scalar sigmaAbsG( cloud.radCalc(cellP, d, aP, nP, pow4(T_), dt) );
	if (cloud.coupledRadiation())
	{
	    ancp += sigmaAbsG;
	}
	//</fmglobal>
