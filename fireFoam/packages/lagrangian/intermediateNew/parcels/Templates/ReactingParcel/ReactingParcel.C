...
)
{
    ...
    // - AK if we have an in-active parcel below the mass threshold, 
    //   it does not do anything here, we want to keep it so it can
    //   act as film parcel
    
    if (np0*mass0 < cloud.constProps().minParcelMass() && !this->active())
    {
        Info << "Skipping calc() for inactive parcel" << nl
             << "In cell: " << this->cell() << nl
             << "With temperature: " << T0 << nl
             << "Diameter: " << d0 << nl
             << "Mass: " << mass0*np0 << nl
             << "min. mass threshold: " << cloud.constProps().minParcelMass() << nl
             << "Because it is too small" << endl;
        return;
    }
    ...
    if (np0*mass1 < cloud.constProps().minParcelMass())
    {
        ...
        //- AK the inactive parcles are film parcels, we don't want to 
        //  absorb these.
        if (!this->active())
        {
            return;
